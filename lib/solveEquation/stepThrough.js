...
function stepThrough(leftNode, rightNode, comparator, debug=false) {
  ...
  do {
    ...
    if (steps.length > 0) {
      ...
      equation = new Equation(lastStep.newEquation.leftNode, lastStep.newEquation.rightNode, comparator);
      //equation = Equation.createEquationFromString(lastStep.newEquation.ascii(), equation.comparator);
    }
...
function solveConstantEquation(equation, debug, steps=[]) {
  ...
  if (steps.length > 0) {
    const lastStep = steps[steps.length - 1];
    //https://github.com/josdejong/mathjs/issues/1431
    equation = new Equation(lastStep.newEquation.leftNode, lastStep.newEquation.rightNode, equation.comparator);
    //equation = Equation.createEquationFromString(lastStep.newEquation.ascii(), equation.comparator);
  }
